package com.joaopedroafluz.devfood.core.springfox.adapter;
...
public class TypeNameExtractorAdapter extends TypeNameExtractor {
    ...
    public TypeNameExtractorAdapter(TypeResolver typeResolver, PluginRegistry<TypeNameProviderPlugin,
            DocumentationType> typeNameProviders, EnumTypeDeterminer enumTypeDeterminer) {
    ...
    private String genericTypeName(ResolvedType resolvedType, ModelContext context) {
        ...
        String simpleName =
                (String) Optional.fromNullable(Types.typeNameFor(erasedType)).or(this.typeName(nameContext));
        ...
        for (int index = 0; index < erasedType.getTypeParameters().length; ++index) {
            ...
            if (first) {
                ...
                sb.append(String.format("%s%s", namingStrategy.getTypeListDelimiter(), this.innerTypeName(typeParam,
                        context)));
            }
        }
    ...
    private String innerTypeName(ResolvedType type, ModelContext context) {
        return type.getTypeParameters().size() > 0 && type.getErasedType().getTypeParameters().length > 0 ?
                this.genericTypeName(type, context) : this.simpleTypeName(type, context);
    }
    ...
    private String simpleTypeName(ResolvedType type, ModelContext context) {
        ...
        if (type instanceof ResolvedPrimitiveType) {
            ...
            return String.format("Array%s%s%s", namingStrategy.getOpenGeneric(),
                    this.simpleTypeName(type.getArrayElementType(), context), namingStrategy.getCloseGeneric());
        } else {
    ...
    private String typeName(ModelNameContext context) {
        TypeNameProviderPlugin selected =
                (TypeNameProviderPlugin) this.typeNameProviders.getPluginOrDefaultFor(context.getDocumentationType(),
